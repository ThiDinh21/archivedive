# Changelog

## [0.2.0] — 2026-06-18

- Card name autocomplete with tab/right-arrow accept
- Parallel fetching for OR queries
- Cache clear shortcut (`ctrl+delete`)
- Fix name search missing expected cards (e.g. `t:champion s` not showing Silvie)
- Fix OR queries only returning first page of results
- Fix incorrect API parameter names
- Improved loading and status bar behavior
- Adjusted table layout and page size

## [0.1.2] — 2026-06-18

- Rarity comparison operators (`r>c`, `r>=sr`, etc.)
- Partial keyword matching — `kw:float` now matches **Floating Memory**
- Fix app freeze when using wl-copy for clipboard

## [0.1.1] — 2026-05-31

- Documentation fixes

## [0.1.0] — 2026-05-31

- Initial release
